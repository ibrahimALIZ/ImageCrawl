package com.alizinhouse.imagecrawl.util;
...
public final class ByteArrayBuffer implements Serializable {
	...
	/**
	 * Appends <code>len</code> bytes to this buffer from the given source array
	 * starting at index <code>off</code>. The capacity of the buffer is
	 * increased, if necessary, to accommodate all <code>len</code> bytes.
	 * 
	 * @param b
	 ...
	 *             if <code>off</code> if out of range, <code>len</code> is
	 *             negative, or <code>off</code> + <code>len</code> is out of
	 *             range.
	 */
	...
	/**
	 * Appends <code>b</code> byte to this buffer. The capacity of the buffer is
	 * increased, if necessary, to accommodate the additional byte.
	 * 
	 * @param b
	...
	/**
	 * Appends <code>len</code> chars to this buffer from the given source array
	 * starting at index <code>off</code>. The capacity of the buffer is
	 * increased if necessary to accommodate all <code>len</code> chars.
	 * <p>
	...
	/**
	 * Appends <code>len</code> chars to this buffer from the given source char
	 * array buffer starting at index <code>off</code>. The capacity of the
	 * buffer is increased if necessary to accommodate all <code>len</code>
	 * chars.
	 * <p>
	 * The chars are converted to bytes using simple cast.
	 * 
	 ...
	 * index. The index argument must be greater than or equal to <code>0</code>
	 * , and less than the length of this buffer.
	 * 
	 * @param i
	 *            the index of the desired byte value.
	 ...
	 * available for newly appended bytes, beyond which an allocation will
	 * occur.
	 * 
	 ...
	 * is allocated with greater capacity. If the <code>required</code> argument
	 * is non-positive, this method takes no action.
	 * 
	 * @param required
	 ...
	 * <code>beginIndex</code> and finishing at <code>endIndex</code>. If no
	 * such byte occurs in this buffer within the specified bounds,
	 ...
	 * . If the <code>beginIndex</code> is greater than the
	 * <code>endIndex</code>, <code>-1</code> is returned.
	 * 
	 * @param b
	 ...
	 *         within the given bounds, or <code>-1</code> if the byte does not
	 *         occur.
	 * 
	 * @since 4.1
	 */
	...
	/**
	 * Returns the index within this buffer of the first occurrence of the
	 * specified byte, starting the search at <code>0</code> and finishing at
	 * {@link #length()}. If no such byte occurs in this buffer within those
	 * bounds, <code>-1</code> is returned.
	 * 
	 * @param b
	 *            the byte to search for.
	 * @return the index of the first occurrence of the byte in the buffer, or
