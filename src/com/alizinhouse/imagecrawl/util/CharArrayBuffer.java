...
public final class CharArrayBuffer implements Serializable {
	 ...
	 *             if <code>off</code> is out of range, <code>len</code> is
	...
	/**
	 * Appends all chars to this buffer from the given source buffer starting at
	 * index <code>0</code>. The capacity of the destination buffer is
	 ...
	 * such character occurs in this buffer within the specified bounds,
	 ...
	 * from <code>beginIndex</code> and extends to the last non-whitespace
	 * character with the index lesser than <code>endIndex</code>.
	 * 
	 * @param beginIndex
	 *            the beginning index, inclusive.
	 * @param endIndex
	...
	public String substringTrimmed(int beginIndex, int endIndex) {
		if (beginIndex < 0) {
			throw new IndexOutOfBoundsException("Negative beginIndex: "
					+ beginIndex);
		}
		if (endIndex > this.len) {
		...
		while (beginIndex < endIndex
				&& HTTP.isWhitespace(this.buffer[beginIndex])) {
			beginIndex++;
		}
		while (endIndex > beginIndex
				&& HTTP.isWhitespace(this.buffer[endIndex - 1])) {
			endIndex--;
		}
		return new String(this.buffer, beginIndex, endIndex - beginIndex);
	}
